    ...
    useEffect(() => {
        ...
        let tm = tdata.map(item =>
            <tr className="bg-white border-b hover:bg-gray-50 font-bold key={item.movieCd}">
                ...
                <td className="px-6 py-2">
                    {parseInt(item.audiInten) >0 ? <span className="text-red-600">▲</span> : parseInt(item.rankInten) < 0 ? <span className="text-blue-600">▼</span> : '-'}
                    {parseInt(item.audiInten) != 0 && Math.abs(item.rankInten)}
                </td>
            </tr>)
